...
export interface InputOptions {
	cache?: boolean | RollupCache;  // 是否启用缓存以提高构建性能，可以指定一个现有的缓存对象
	context?: string;  // 模块执行上下文
	experimentalCacheExpiry?: number;  // 缓存过期时间（毫秒），用于控制缓存数据的有效期
	experimentalLogSideEffects?: boolean;  // 是否记录模块的副作用
	external?: ExternalOption;  // 指定哪些模块不应被打包，可以是一个字符串数组、函数或者正则表达式
	input?: InputOption;  // 指定入口文件或文件路径，可以是一个文件路径字符串或者包含多个入口的对象
	logLevel?: LogLevelOption;  // 日志级别，控制日志输出的详细程度
	makeAbsoluteExternalsRelative?: boolean | 'ifRelativeSource';  // 是否将外部模块路径转换为相对路径
	maxParallelFileOps?: number;  // 最大并行文件操作数，用于控制文件系统操作的并行数量
	moduleContext?: ((id: string) => string | NullValue) | { [id: string]: string };  // 模块执行上下文的映射，用于设置模块的执行上下文
	onLog?: LogHandlerWithDefault;  // 自定义日志处理程序
	onwarn?: WarningHandlerWithDefault;  // 自定义警告处理程序
	perf?: boolean;  // 是否开启性能分析
	plugins?: InputPluginOption;  // 插件配置选项，用于指定要使用的插件
	preserveEntrySignatures?: PreserveEntrySignaturesOption;  // 是否保留入口模块的签名信息
	preserveSymlinks?: boolean;  // 是否保留符号链接
	shimMissingExports?: boolean;  // 是否自动为模块添加导出
	strictDeprecations?: boolean;  // 是否启用严格的废弃警告
	treeshake?: boolean | TreeshakingPreset | TreeshakingOptions;  // 是否启用Tree-Shake优化，可以是布尔值、预设值或自定义选项
	watch?: WatcherOptions | false;  // 监听模式配置选项，用于启动监听模式并指定监听选项
}
